...
impl Subject for KeySubject {
    ...
    async fn sign(&self, message: &String) -> Result<Vec<u8>> {
        Ok(self.key.sign(message.as_bytes()).to_vec())
    }
}
...
impl Validator for KeyValidator {
    async fn public_key(&self, kid: &String) -> Result<Vec<u8>> {
        let key = resolve(kid).map_err(|_| anyhow!("Failed to resolve the key identifier"))?;
        let authentication_method = key
